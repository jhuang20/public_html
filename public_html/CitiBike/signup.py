...
def getmessage():
    return ''.join(readlines("event.csv"))+"""<br><p> <b>Master Event List(temp)</b></p>"""
...
def getEventCSV():#gets event DATE
    csvfile = open("event.csv", "r")#opens the file
    red=csv.reader(csvfile, dialect='excel')
    final=''
    for row in red:
        ret=row.pop(0)#gives you the DATE of the Event
        final+="""

        <button class="w3-button w3-black w3-right w3-section" onmouseover="expand(1)"">"""+ret+"""</button><br>
        """
    return final+""
def getEvent():
    return ""
...
function count() {
    ...
    if( typeof count.counter == 'undefined' || count.counter>1 ) {
        count.counter = 0;
    }
    count.counter++;
...
document.getElementById("attrib").innerHTML="<p class='w3-large'>"""+"""Test</p>";

}

else {
document.getElementById("attrib").innerHTML="<p class='w3-large'>THis ain't working rn but it will soon!"""+"""</p>";
...
var child = document.getElementById("attrib");
parent.removeChild(child);
var para = document.createElement("p", class="w3-center w3-xxlarge", id="attrib");
var node = document.createTextNode("Sorry, at this moment, there are no signups available!");
